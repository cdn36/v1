...
function startTask() {
    ...
    jQuery.get(siteUrl + '/assets/data/backlink.dblinks', function (data) {
        myArr = data.split('\n');
        if (myArr.length < 2) {
...
function reUsetool() {
    ...
    jQuery("#resultBox").css({ "display": "none" });
    jQuery("#resultBox").hide();
    jQuery("#resultBox").fadeOut();
...
function make(domainID) {
    ...
    jQuery.post(axPath, { backlink: '1', sitelink: c_link }, function (data) {
        if (data == '1')
