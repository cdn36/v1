$('#ls-ajaxModal-form').submit(function (f) {
    f.preventDefault();
    ...
    if ($('#g-recaptcha-response').length) {
        ...
        if (mmRecaptcha === "") {
            alert('Please check the recaptcha'); /*ls.alertMsg({ message: 'Please check the recaptcha', type: 'info', timeout: 8000 });*/
            return;
        }
    }

    setTimeout(function () {
        var $modal = $('#ls-empty-modal');
        $modal.find('.modal-title').text('Thank you for using!');
        var $$notfmodal = $modal.find('.modal-body').html('');

        $$notfmodal.append(
            $('<div />', { id: 'ls-op-result' }).html($('<p />', { style: 'font-size: 2.2em', class: 'display-4' }).html('<i class="fa fa-spinner fa-pulse fa-fw"></i> <strong>Sending...</strong>'))
        );
        /*$$notfmodal.append('<div class="table-responsive"> <table class="table table-sm"> <thead> <tr> <th scope="col">Service Name</th> <th scope="col">Price</th> <th scope="col">Maximum</th> </tr> </thead> <tbody> <tr> <th scope="row">FB Page Likes</th> <td>$5 per 1000</td> <td>40,000</td> </tr> <tr> <th scope="row">FB Page Likes w/ "see first"</th> <td>$9 per 1000</td> <td>40,000</td> </tr> <tr> <th scope="row">FB Profile Followers</th> <td>$4 per 1000</td> <td>40,000</td> </tr> <tr> <th scope="row">FB Profile Followers /w "see first"</th> <td>$7 per 1000</td> <td>40,000</td> </tr> <tr> <th scope="row">FB Page Rating/Review</th> <td>$10 per 50</td> <td>40,000</td> </tr> <tr> <th scope="row">FB Post Likes</th> <td>$3 per 1000</td> <td>40,000</td> </tr> <tr> <th scope="row">FB Post Comments</th> <td>$3 per 1000</td> <td>40,000</td> </tr> <tr> <th scope="row">FB Video Views</th> <td>$3 per 1000</td> <td>2,000,000</td> </tr> </tbody> </table> </div> <a href="https://m.me/leloweb" class="btn btn-warning btn-block">Contact us to purchase!</a>'); */
        $$notfmodal.append('<br/>');
        $$notfmodal.append(
            $('<p />', { class: 'lead mt-3' }).text('Having Trouble using our application?'),
            $('<a />', { class: 'btn btn-info btn-md', target: '_blank', href: ajaxReqData.siteUrl + '/contact' }).text('Help / Support')
        );
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.success === true) {
                ...
                var $opresult = $('#ls-op-result');
                $opresult.html($('<p />', { style: 'font-size: 2.2em', class: 'display-4' }).html('<strong class="text-success"> ' + mesDataShow + ' !</strong>'));
                ...
                var $opresult = $('#ls-op-result');
                $opresult.html($('<p />', { style: 'font-size: 2.2em', class: 'display-4' }).html('<strong class="text-warning">' + data.type + ' </strong><br/><p>' + data.message + '</p>'));
